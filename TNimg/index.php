...
declare(strict_types=0); // Ustawiono na 1
...
$tn_app_name = htmlspecialchars($tn_ustawienia_globalne['nazwa_strony'] ?? 'TNiMAG', ENT_QUOTES, 'UTF-8');
...
$tn_page_map = [
    ...
    'produkty' => 'products',
    'produkty/podglad' => 'product_preview',
    'zamowienia' => 'orders',
    'zamowienia/podglad' => 'order_preview', 
    ...
    'zwroty/edytuj' => 'return_form_edit',
    'zwroty/podglad' => 'return_preview', 
    'kurierzy' => 'couriers_list',
    'produkty/pojazdy' => 'manage_vehicles', 
    'pojazdy' => 'vehicles', 
];
...
$path_string = trim($tn_relative_path, '/'); 
...
if (isset($tn_page_map[$path_string])) {
    ...
    elseif ($first_part === 'produkty' && $second_part === 'pojazdy' && isset($tn_path_parts[2])) {
         $id = filter_var($third_part, FILTER_VALIDATE_INT);
         if ($id !== false) {
...
$tn_produkt_podgladu = null; 
$tn_zamowienie_podgladu = null;
$tn_zwrot_podgladu = null;
$tn_produkty_dane = null; 
$tn_zamowienia_dane = null; 
$tn_zwroty_dane = null;
$tn_lista_pojazdow = []; 
...
$versionRegex2 = '/^\s*(.+?)\s+\(([^)]+?)\)\s*,\s*(\d*)\s*,\s*(\d*)\s*,\s*(\d*)\s*,\s*(\d{4}\.\d{2})\s*-\s*(.*?)\s*$/';
            ...
            foreach ($lines as $line) {
                ...
                elseif ($currentMakeModel !== null && preg_match('/^\s+/', $line, $matches)) { // Corrected regex match
                    ...
                    $version_data = [
                        ...
                        'year_end' => htmlspecialchars(trim($matches[7] ?? ''), ENT_QUOTES, 'UTF-8') ?: '...',
                    ];
            ...
            $tn_lista_pojazdow = $unique_vehicles_map; 
            break;
...
$tn_view_config = [
    ...
    'vehicles' => ['file' => 'pages/tn_vehicles_list.php', 'title' => 'Baza Pojazdów'], 
];
...
if ($tn_biezaca_strona_id === 'login_page') {
    ...
    if (!empty($tn_komunikaty_flash)) {
        echo '<div class="tn-flash-container position-fixed top-0 end-0 p-3" style="opacity: 66%; z-index: 1056;">';
